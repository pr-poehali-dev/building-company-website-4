    ...
    <div className="min-h-screen bg-white text-neutral-900 font-body">
      ...
      <header className="fixed top-0 inset-x-0 z-50 bg-white/80 backdrop-blur-md border-b border-neutral-100">
        <div className="container mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <Button className="hidden lg:inline-flex rounded-none bg-neutral-900 hover:bg-neutral-700 px-6">
            Обсудить проект
          </Button>
        ...
        {menuOpen && (
          <nav className="lg:hidden bg-white border-t border-neutral-100 px-6 py-4 flex flex-col gap-4">
            {navLinks.map((l) => (
              <a key={l.href} href={l.href} onClick={() => setMenuOpen(false)} className="text-neutral-700">
                {l.label}
              </a>
            ))}
          </nav>
      ...
      <section id="hero" className="relative pt-20 min-h-screen flex items-center">
        <div className="container mx-auto px-6 py-24 grid lg:grid-cols-2 gap-16 items-center">
          <div className="animate-fade-in">
            <p className="text-sm uppercase tracking-[0.3em] text-neutral-400 mb-6">Строительная компания</p>
            <h1 className="font-display text-5xl md:text-7xl font-600 leading-[1.05] tracking-tight text-balance">
              Строим пространства, в которых хочется жить
            </h1>
            <p className="mt-8 text-lg text-neutral-500 max-w-md leading-relaxed">
              Полный цикл строительства и отделки — от первого эскиза до сдачи объекта под ключ.
            </p>
            <div className="mt-10 flex flex-wrap gap-4">
              <Button className="rounded-none bg-neutral-900 hover:bg-neutral-700 px-8 h-12">
                Смотреть проекты
              </Button>
              <Button variant="outline" className="rounded-none border-neutral-300 px-8 h-12">
                Связаться с нами
              </Button>
            </div>
            <div className="mt-16 flex gap-12">
              {[['15', 'лет на рынке'], ['250+', 'объектов'], ['98%', 'в срок']].map(([n, t]) => (
                <div key={t}>
                  <div className="font-display text-4xl font-600">{n}</div>
                  <div className="text-sm text-neutral-400 mt-1">{t}</div>
                </div>
              ))}
            </div>
          </div>
          <div className="relative animate-fade-in-slow">
            ...
            <div className="absolute -bottom-6 -left-6 bg-white p-6 shadow-xl hidden md:block">
              <div className="font-display text-3xl font-600">2024</div>
              <div className="text-sm text-neutral-400">лучший проект года</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="services" className="py-28 bg-neutral-50">
        <div className="container mx-auto px-6">
          <div className="max-w-2xl mb-16">
            <p className="text-sm uppercase tracking-[0.3em] text-neutral-400 mb-4">Услуги</p>
            <h2 className="font-display text-4xl md:text-5xl font-600 tracking-tight">Что мы делаем</h2>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-px bg-neutral-200">
            {services.map((s) => (
              <div key={s.title} className="bg-white p-10 group hover:bg-neutral-900 transition-colors duration-300">
                <Icon name={s.icon} size={32} className="text-neutral-900 group-hover:text-white transition-colors" />
                <h3 className="font-display text-xl font-500 mt-6 group-hover:text-white transition-colors">{s.title}</h3>
                <p className="text-sm text-neutral-500 mt-3 leading-relaxed group-hover:text-neutral-300 transition-colors">{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container mx-auto px-6">
          <div className="flex flex-wrap items-end justify-between gap-6 mb-16">
            <div className="max-w-2xl">
              <p className="text-sm uppercase tracking-[0.3em] text-neutral-400 mb-4">Портфолио</p>
              <h2 className="font-display text-4xl md:text-5xl font-600 tracking-tight">Завершённые проекты</h2>
            </div>
            <Button variant="outline" className="rounded-none border-neutral-300 px-6">Все проекты</Button>
          </div>
            ...
            {projects.map((p) => (
              <div key={p.title} className="group cursor-pointer">
                ...
                <div className="flex items-start justify-between mt-5">
                  <div>
                    <h3 className="font-display text-xl font-500">{p.title}</h3>
                    <p className="text-sm text-neutral-400 mt-1">{p.tag}</p>
                  </div>
      ...
      {/* About */}
      <section id="about" className="py-28 bg-neutral-900 text-white">
        <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
          <img src={IMG_2} alt="О компании" className="w-full aspect-[4/3] object-cover" />
          <div>
            <p className="text-sm uppercase tracking-[0.3em] text-neutral-500 mb-4">О компании</p>
            ...
            <div className="mt-10 grid grid-cols-2 gap-8">
              {[['Собственная техника', 'Wrench'], ['Гарантия 5 лет', 'ShieldCheck'], ['Фиксированная цена', 'BadgeRussianRuble'], ['Сдача в срок', 'CalendarCheck']].map(([t, ic]) => (
                <div key={t} className="flex items-center gap-3">
                  <Icon name={ic} size={22} className="text-neutral-500" />
      ...
      <section id="reviews" className="py-28 bg-neutral-50">
        <div className="container mx-auto px-6">
          <div className="max-w-2xl mb-16">
            <p className="text-sm uppercase tracking-[0.3em] text-neutral-400 mb-4">Отзывы</p>
            <h2 className="font-display text-4xl md:text-5xl font-600 tracking-tight">Нам доверяют</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {reviews.map((r) => (
              <div key={r.name} className="bg-white p-10 border border-neutral-100">
                <div className="flex gap-1 mb-6">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Icon key={i} name="Star" size={16} className="text-neutral-900 fill-neutral-900" />
                  ))}
                </div>
                <p className="text-neutral-600 leading-relaxed">«{r.text}»</p>
                <div className="mt-8 pt-6 border-t border-neutral-100">
                  <div className="font-display text-lg font-500">{r.name}</div>
      ...
      <section id="contacts" className="py-28">
        <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-16">
          <div>
            <p className="text-sm uppercase tracking-[0.3em] text-neutral-400 mb-4">Контакты</p>
            <h2 className="font-display text-4xl md:text-5xl font-600 tracking-tight">Обсудим ваш проект</h2>
            ...
            <div className="mt-10 space-y-5">
              {[['Phone', '+7 (495) 000-00-00'], ['Mail', 'hello@stroygrad.ru'], ['MapPin', 'Москва, ул. Строителей, 12']].map(([ic, t]) => (
                <div key={t} className="flex items-center gap-4">
                  <Icon name={ic} size={20} className="text-neutral-900" />
          ...
          <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
            <input className="w-full border border-neutral-200 px-5 h-12 outline-none focus:border-neutral-900 transition-colors" placeholder="Ваше имя" />
            <input className="w-full border border-neutral-200 px-5 h-12 outline-none focus:border-neutral-900 transition-colors" placeholder="Телефон" />
            <textarea rows={4} className="w-full border border-neutral-200 px-5 py-3 outline-none focus:border-neutral-900 transition-colors resize-none" placeholder="О проекте" />
            <Button className="w-full rounded-none bg-neutral-900 hover:bg-neutral-700 h-12">
              Отправить заявку
            </Button>
          </form>
        </div>
